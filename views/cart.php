    ...
    <div class="row">
        <div class="col-md-8 bg-white">
            <table class="table table-stripped">
                ...
                <tbody>
                    <?php foreach($_SESSION as $name => $product):?> <!-- get product form session -->
                    <?php if(substr($name,0,9) == "products_"):?> <!--test title == product --> 
                    <tr>
                ...
                <?php elseif(isset($_SESSION["count"]) && $_SESSION["count"] > 0 ):?>
                    <a href="<?php echo BASE_URL;?>login" class="btn btn-link">Conncetez vous pour terminer vos achats</a>
                <?php endif;?>
        </div>
...
<script>
    ...
    let finalAmount = Math.floor(amount / 8.86);
    paypal.Buttons({
        ...
        return actions.order.create({
            purchase_units: [{
            amount: {
                value: finalAmount.toString() // transform the amount to string
            }
            }]
        });
        },
